...
public class SwaggerConfig implements WebMvcConfigurer {
  ...
  public Docket createRestApi() {
    return new Docket(DocumentationType.OAS_30)
        ...
        // 定义全局请求参数: 适用于为请求的Header添加一个认证参数，每次请求的时候，需要手动输入token的场景
   ...
   * @return
   */
  protected List<RequestParameter> authorizationParameter() {
  ...
  protected ApiInfo apiInfo() {
    return new ApiInfoBuilder()
        ...
        .contact(new Contact("xue.zeng", null, "xue.zeng@outlook.com"))
        .version(
            "Application Version: "
  ...
  @SafeVarargs
  protected final <T> Set<T> newHashSet(T... ts) {
    if (ts.length > 0) {
